...
export const initializeSocket = (httpServer: HttpServer) => {
  ...
  io.use(async (socket, next) => {
    const token = socket.handshake.auth.token; // this is what user will send from client(token)
    if (!token) return next(new Error("Authentication error"));

    try {
      const session = await verifyToken(token, { secretKey: process.env.CLERK_SECRET_KEY! });  //Verify token

      const clerkId = session.sub;  // get clerk user id from the session

      const user = await User.findOne({ clerkId });  //find user from the D.B
